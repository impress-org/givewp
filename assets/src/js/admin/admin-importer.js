 ...
 * @since 1.8.14
 */
import {GiveErrorAlert} from '../plugins/modal';
...
function give_on_core_settings_import_start() {
    const import_step =
        'body.give_forms_page_give-tools .give-tools-import-tab #give-import-core-settings-form table.step-2';
...
function give_import_core_settings_json_is_valid() {
    const import_step =
        'body.give_forms_page_give-tools .give-tools-import-tab #give-import-core-settings-form table.step-1 .is_json_valid';
    if (jQuery(import_step).length > 0) {
...
function give_start_importing_donations() {
    const import_step =
        'body.give_forms_page_give-tools .give-tools-import-tab #give-import-donations-form table.step-3';
    if (jQuery(import_step).length > 0) {
...
function give_import_donation_csv_not_valid() {
    const import_step =
        'body.give_forms_page_give-tools .give-tools-import-tab #give-import-donations-form table.step-2 .csv_not_valid';
    if (jQuery(import_step).length > 0) {
        window.location = jQuery(import_step).val();
    }
}
...
function give_import_donation_valid_csv() {
    const import_step =
        'body.give_forms_page_give-tools .give-tools-import-tab #give-import-donations-form table.step-1 .is_csv_valid';
...
function give_on_donation_import_ajax() {
    ...
    const fields = JSON.stringify(Object.fromEntries(formData));
...
function give_import_donation_required_fields_check() {
    const required_fields = document.querySelector(
    ...
    if (required_fields !== 'undefined' && required_fields !== null) {
        ...
        if (select_fields.includes('email')) {
            email = true;
            document
                .querySelector('.give-import-donation-required-email .dashicons')
                .classList.remove('dashicons-no-alt');
        ...
        if (select_fields.includes('first_name')) {
            first_name = true;
            document
                .querySelector('.give-import-donation-required-first .dashicons')
                .classList.remove('dashicons-no-alt');
        ...
        if (select_fields.includes('amount')) {
            amount = true;
            document
                .querySelector('.give-import-donation-required-amount .dashicons')
                .classList.remove('dashicons-no-alt');
        ...
        if (select_fields.includes('form_id') || select_fields.includes('form_title')) {
            form = true;
            document
                .querySelector('.give-import-donation-required-form .dashicons')
                .classList.remove('dashicons-no-alt');
