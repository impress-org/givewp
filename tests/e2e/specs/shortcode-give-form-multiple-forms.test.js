...
describe( 'Display Option: All fields', () => {
	...
	beforeAll( async () => await page.goto( `${give.utility.vars.rootUrl}/give-donation-form-shortcode-with-multiple-forms/` ) )

	give.utility.fn.verifyExistence( page, [

		{
			desc: 'verify form title as "Reveal Form"',
			selector: '.give-display-onpage .give-form-title',
			innerText: 'Reveal Form',
		},

		{
			desc: 'verify form content',
			selector: '.give-display-onpage .give-form-content-wrap p',
			innerText: 'Form Content of the Reveal Form. A moderately long description.',
		},
	...
	it( 'INTERACTION: fill first form and donate', async () => {
		...
		await expect( page ).toFillForm( '.give-display-onpage .give-form', {
			give_first: 'Jim',
			give_last: 'Halpert',
			give_email: 'jim.halpert@gmail.com',
		}, 10000 )
	...
	it( 'INTERACTION: redirect to the shortcode page', async () => {
		await Promise.all([
			page.goto( `${give.utility.vars.rootUrl}/give-donation-form-shortcode-with-multiple-forms/` )
		])
	})
