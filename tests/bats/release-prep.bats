...
#
# Steps 1 and 2 are backed by stellarwp/pup (replace-version / replace-tbd);
# the script downloads the pup phar on demand, so these tests require network
...
@test "dry-run runs all three steps in order and completes" {
    ...
    # pup replace-version has no dry-run, so step 1 is announced as skipped.
    assert_output --partial "version bump skipped"
