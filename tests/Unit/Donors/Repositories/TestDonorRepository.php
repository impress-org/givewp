...
class TestDonorRepository extends TestCase
{
    ...
    public function testUpsertMetaShouldInsertNewMeta(): void
    {
        $donor = Donor::factory()->create();
        $repository = new DonorRepository();
        $repository->upsertMeta($donor->id, 'test_key', 'test_value');
    ...
    public function testUpsertMetaShouldNotDuplicateMeta(): void
    {
        $donor = Donor::factory()->create();
        $repository = new DonorRepository();
        $repository->upsertMeta($donor->id, 'test_key', 'Test Value One');
        $repository->upsertMeta($donor->id, 'test_key', 'Test Value Two');
