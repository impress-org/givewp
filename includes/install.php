...
/**
 * Display notice to outdated PHP versions
 *
 * @since 1.8.9
 *
 * @return string
...
function give_display_outdated_php_version_notice() {

	// Check for outdated PHP Versions, if outdated show dismissable notice.
	if( SUPPORT_FROM_PHP_VERSION > give_get_current_php_version() ) {
		...
		$html .= '<p>' . __( 'Hey, we\'ve noticed that you\'re running an outdated version of PHP. PHP is the programming language that WordPress and Yoast SEO are built on. The version that is currently used for your site is no longer supported. Newer versions of PHP are both faster and more secure. In fact, your version of PHP no longer receives security updates, which is why we\'re sending you to this notice.', 'give' ) . '</p>';
		...
		$html .= '<p>' . sprintf( __( 'If you cannot upgrade your PHP version yourself, you can send an email to your host. We have %1$sexamples here%2$s. If they don\'t want to upgrade your PHP version, we would suggest you switch hosts. Have a look at one of the recommended %3$sWordPress hosting partners%4$s.', 'give' ), '<a href="https://yoa.st/wh" target="_blank">', '</a>', sprintf( '<a href="%1$s" target="_blank">', esc_url( 'https://wordpress.org/hosting/' ) ), '</a>' ) . '</p>';
		$html .= '</div>';

		echo apply_filters( 'give_outdated_php_version_notice_message', $html);
	}
}
add_action( 'admin_notices', 'give_display_outdated_php_version_notice' ) ;