...
function give_run_install() {
	...
	if ( $current_version ) {
		update_option( 'give_version_upgraded_from', $current_version, 'no' );
	}
	...
	update_option( 'give_settings', array_merge( $give_options, $options ), 'no' );
	...
	$api = new Give_API();
	$api->add_endpoint();
	update_option( 'give_default_api_version', 'v' . $api->get_version(), 'no' );
...
function give_after_install() {
	...
	if ( false === $give_table_check || current_time( 'timestamp' ) > $give_table_check ) {
		...
		update_option( '_give_table_check', ( current_time( 'timestamp' ) + WEEK_IN_SECONDS ), 'no' );
