		...
		<?php if ( $form->is_close_donation_form() ) {
			...
			<form id="give-form-<?php echo $form_id; ?>" class="<?php echo $form_classes; ?>"
				...
				<input type="hidden" name="give-form-minimum"
					   value="<?php echo give_format_amount( give_get_form_minimum_price( $form->ID ), true, false ); ?>"/>
...
function give_output_donation_amount_top( $form_id = 0, $args = array() ) {
	...
	$default_amount      = give_format_amount( give_get_default_form_amount( $form_id ), true, false );
...
function give_output_levels( $form_id ) {
	...
	switch ( $display_style ) {
		case 'buttons':
			...
			foreach ( $prices as $price ) {
				$level_text    = apply_filters( 'give_form_level_text', ! empty( $price['_give_text'] ) ? $price['_give_text'] : give_currency_filter( give_format_amount( $price['_give_amount'], true, false ) ), $form_id, $price );
				...
				$output .= '<li>';
				$output .= '<button type="button" data-price-id="' . $price['_give_id']['level_id'] . '" class=" ' . $level_classes . '" value="' . give_format_amount( $price['_give_amount'], true, false ) . '">';
				$output .= $level_text;
				$output .= '</button>';
				$output .= '</li>';
		...
		case 'radios':
			...
			foreach ( $prices as $price ) {
				...
				$output .= '<li>';
				$output .= '<input type="radio" data-price-id="' . $price['_give_id']['level_id'] . '" class="' . $level_classes . '" name="give-radio-donation-level" id="give-radio-level-' . $price['_give_id']['level_id'] . '" ' . ( ( isset( $price['_give_default'] ) && $price['_give_default'] === 'default' ) ? 'checked="checked"' : '' ) . ' value="' . give_format_amount( $price['_give_amount'], true, false ) . '">';
		...
		case 'dropdown':
			...
			foreach ( $prices as $price ) {
				...
				$output .= '<option data-price-id="' . $price['_give_id']['level_id'] . '" class="' . $level_classes . '" ' . ( ( isset( $price['_give_default'] ) && $price['_give_default'] === 'default' ) ? 'selected="selected"' : '' ) . ' value="' . give_format_amount( $price['_give_amount'], true, false ) . '">';
				$output .= $level_text;
				$output .= '</option>';
