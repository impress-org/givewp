...
function give_manual_payment( $purchase_data ) {
	...
	*/
	...
	if ( $payment ) {
		...
		give_record_gateway_error( __( 'Payment Error', 'edd' ), sprintf( __( 'Payment creation failed while processing a manual (free or test) purchase. Payment data: %s', 'edd' ), json_encode( $payment_data ) ), $payment );
