...
class Give_DB_Form_Meta extends Give_DB {
	...
	public function get_meta( $form_id = 0, $meta_key = '', $single = false ) {
		...
		if ( $this->raw_result ) {
			if ( ! ( $value = get_metadata( 'form', $form_id, $meta_key, false ) ) ) {
				$value = '';
			}
			...
			$value = get_metadata( 'form', $form_id, $meta_key, $single );
		}
	...
	public function __call( $name, $arguments ) {
		...
		switch ( $name ) {
			case '__add_meta':
				...
				return $this->add_meta( $form_id, $meta_key, $meta_value, $unique );
			...
			case '__get_meta':
				...
				return $this->get_meta( $form_id, $meta_key, $single );
			...
			case '__update_meta':
				...
				return $this->update_meta( $form_id, $meta_key, $meta_value );
			...
			case '__delete_meta':
				...
				return $this->delete_meta( $form_id, $meta_key, $meta_value, $delete_all );
		}
	}
