...
class Give_Donation_Form_Block {
	...
	public function enqueue_block_editor_assets() {
		...
		wp_enqueue_script(
			...
			GIVE_PLUGIN_URL . 'includes/blocks/donation-form/build/index.js',
			array( 'wp-blocks', 'wp-i18n', 'wp-element' ),
			GIVE_VERSION
		);

		wp_enqueue_style(
			'give-donation-form-block-editor',
			GIVE_PLUGIN_URL . 'includes/blocks/donation-form/build/style.css',
			array( 'wp-blocks' ),
			GIVE_VERSION
		);

		// Localize vars from PHP.
		wp_localize_script( 'give-donation-form-block', 'give_blocks_vars', array(
