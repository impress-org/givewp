			...
			foreach ( $options as $value ) {
				...
				switch ( $value['type'] ) {
					...
					case 'image_width' :
						...
						$size             = give_get_image_size( $image_size );
						...
						if ( has_filter( 'give_get_image_size_' . $image_size ) ) {
							$disabled_attr = 'disabled="disabled"';
							$disabled_message = "<p><small>" . __( 'The settings of this image size have been disabled because its values are being overwritten by a filter.', 'give' ) . "</small></p>";
						}

						?><tr valign="top">
							<th scope="row" class="titledesc"><?php echo esc_html( $value['title'] ) ?> <?php echo $disabled_message; ?></th>
							<td class="forminp image_width_settings">
								...
								<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" <?php echo $disabled_attr; ?> id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Hard Crop?', 'give' ); ?></label>
					...
					case 'single_select_page' :
						...
						$args = array(
							...
							'class'            => $value['class'],
							'echo'             => false,
							'selected'         => absint( self::get_option( $option_name, $value['id'] ) )
						);
						...
						?><tr valign="top" class="single_select_page">
							<th scope="row" class="titledesc"><?php echo esc_html( $value['title'] ) ?></th>
							<td class="forminp">
								<?php echo str_replace(' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'give' ) .  "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); ?> <?php echo $description; ?>
							</td>
						</tr><?php
						break;
					...
					case 'single_select_country' :
						$country_setting = (string) self::get_option( $option_name, $value['id'] );
						...
						?><tr valign="top">
							...
							<td class="forminp"><select name="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php esc_attr_e( 'Choose a country&hellip;', 'give' ); ?>" title="<?php esc_attr_e( 'Country', 'give' ) ?>" class="wc-enhanced-select">
								<?php //Give()->countries->country_dropdown_options( $country, $state ); ?>
							</select> <?php echo $description; ?>
							</td>
						</tr><?php
						break;
					...
					case 'multi_select_countries' :
						...
						$selections = (array) self::get_option( $option_name, $value['id'] );

						if ( ! empty( $value['options'] ) ) {
							$countries = $value['options'];
						} else {
							$countries = Give()->countries->countries;
						}
						...
						?><tr valign="top">
							...
							<td class="forminp">
								<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose countries&hellip;', 'give' ); ?>" title="<?php esc_attr_e( 'Country', 'give' ) ?>" class="wc-enhanced-select">
									<?php
										if ( ! empty( $countries ) ) {
