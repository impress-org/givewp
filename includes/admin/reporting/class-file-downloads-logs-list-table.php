...
class EDD_File_Downloads_Log_Table extends WP_List_Table {
	...
	function get_logs() {
		...
		if ( $logs ) {
			foreach ( $logs as $log ) {
				...
				$user_info 	 = maybe_unserialize( $meta[ '_give_log_user_info' ][0] );
				$payment_id  = $meta[ '_give_log_payment_id' ][0];
				$ip 		 = $meta[ '_give_log_ip' ][0];
				$user_id 	 = isset( $user_info['id'] ) ? $user_info['id'] : false;

				if( ! array_key_exists( $log->post_parent, $this->queried_files ) ) {
					$files   = maybe_unserialize( $wpdb->get_var( $wpdb->prepare( "SELECT meta_value from $wpdb->postmeta WHERE post_id = %d and meta_key = 'give_download_files'", $log->post_parent ) ) );
					$this->queried_files[ $log->post_parent ] = $files;
				} else {
					$files   = $this->queried_files[ $log->post_parent ];
				}

				$file_id 	 = (int) $meta[ '_give_log_file_id' ][0];
	...
	function prepare_items() {
		...
		$total_items           = $give_logs->get_log_count( $this->get_filtered_download(), 'file_download', $this->get_meta_query() );
