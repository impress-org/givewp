		...
		<div class="metabox-holder" style="padding-top: 0;">
			<div class="postbox">
				...
				<div class="inside">
					...
					<p class="give_graph_totals"><strong><?php _e( 'Total earnings for period shown: ', 'give' ); echo give_currency_filter( give_format_amount( $earnings_totals ) ); ?></strong></p>
					<p class="give_graph_totals"><strong><?php _e( 'Total sales for period shown: ', 'give' ); echo give_format_amount( $sales_totals, false ); ?></strong></p>
					<?php if( 'this_month' == $dates['range'] ) : ?>
						<p class="give_graph_totals"><strong><?php _e( 'Estimated monthly earnings: ', 'give' ); echo give_currency_filter( give_format_amount( $estimated['earnings'] ) ); ?></strong></p>
						<p class="give_graph_totals"><strong><?php _e( 'Estimated monthly sales: ', 'give' ); echo give_format_amount( $estimated['sales'], false ); ?></strong></p>
					<?php endif; ?>
				</div>
			</div>
		</div>
 ...
 * @return void
*/
function give_reports_graph_of_download( $download_id = 0 ) {
	// Retrieve the queried dates
	$dates = give_get_report_dates();
	...
	<div class="metabox-holder" style="padding-top: 0;">
		<div class="postbox">
			<h3><span><?php printf( __('Earnings Over Time for %s', 'give' ), get_the_title( $download_id ) ); ?></span></h3>
			...
			<div class="inside">
				...
				?>
				<p class="give_graph_totals"><strong><?php _e( 'Total earnings for period shown: ', 'give' ); echo give_currency_filter( give_format_amount( $earnings_totals ) ); ?></strong></p>
				<p class="give_graph_totals"><strong><?php _e( 'Total sales for period shown: ', 'give' ); echo $sales_totals; ?></strong></p>
				<p class="give_graph_totals"><strong><?php printf( __( 'Average monthly earnings: %s', 'give' ), give_currency_filter( give_format_amount( give_get_average_monthly_download_earnings( $download_id ) ) ) ); ?>
				<p class="give_graph_totals"><strong><?php printf( __( 'Average monthly sales: %s', 'give' ), number_format( give_get_average_monthly_download_sales( $download_id ), 0 ) ); ?>
			</div>
		</div>
	</div>
	<?php
...
function give_parse_report_dates( $data ) {
	...
	wp_redirect( add_query_arg( $dates, admin_url( 'edit.php?post_type=download&page=give-reports&view=' . esc_attr( $view ) . '&download-id=' . absint( $id ) ) ) ); give_die();
}
