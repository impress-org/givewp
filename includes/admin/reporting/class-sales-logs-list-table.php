		...
		switch ( $column_name ) {
			...
			case 'status' :
				...
				if ( $payment->mode == 'test' ) {
					$value .= ' <span class="give-item-label give-item-label-orange give-test-mode-transactions-label" data-tooltip="' . esc_attr( 'This payment was made in test mode', 'give' ) . '">' . esc_html__( 'Test', 'give' ) . '</span>';
				}
	...
	public function give_forms_filter() {
		...
		if ( $give_forms ) {
			...
			foreach ( $give_forms as $form ) {
				echo '<option value="' . $form . '"' . selected( $form, $this->get_filtered_give_form() ) . '>' . esc_html__( get_the_title( $form ) ) . '</option>';
			}
			echo '</select>';
		}
	}
