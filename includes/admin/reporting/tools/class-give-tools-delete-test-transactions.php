...
class Give_Tools_Delete_Test_Transactions extends Give_Batch_Export {
	...
	public function process_step() {
		...
		if ( ! $this->can_export() ) {
			wp_die( esc_html( 'You do not have permission to delete test transactions.', 'give' ), esc_html( 'Error', 'give' ), array( 'response' => 403 ) );
		}
		...
		if ( $had_data ) {
			...
			$this->done    = true;
			$this->message = esc_html( 'Test transactions successfully deleted.', 'give' );
