...
function give_generate_pdf( $data ) {
	...
	$pdf->Image( EDD_PLUGIN_URL . 'assets/images/give-logo.png', 205, 10 );
	...
	if ( $downloads ):
		...
		foreach ( $downloads as $download ):
			...
			if ( give_has_variable_prices( $download->ID ) ) {
				...
				$price = html_entity_decode( give_currency_filter( give_get_download_price( $download->ID ) ) );
			}
			...
			$sales = give_get_download_sales_stats( $download->ID );
			$link = get_permalink( $download->ID );
			$earnings = html_entity_decode ( give_currency_filter( give_get_download_earnings_stats( $download->ID ) ) );
