...
function give_setup_email_tags() {
	...
	$email_tags = array(
		array(
			...
			'function'    => 'text/html' == EDD()->emails->get_content_type() ? 'give_email_tag_download_list' : 'give_email_tag_download_list_plain'
		),
		array(
			'tag'         => 'file_urls',
...
function give_email_tag_download_list( $payment_id ) {
	...
	if ( $cart_items ) {
		...
		foreach ( $cart_items as $item ) {
			...
			if ( give_item_quantities_enabled() ) {
			...
			if ( $show_names ) {
				...
				if ( ! empty( $quantity ) && $quantity > 1 ) {
					$title .= "&nbsp;&ndash;&nbsp;" . __( 'Quantity', 'give' ) . ': ' . $quantity;
				}
				...
				if ( $price_id !== null ) {
					$title .= "&nbsp;&ndash;&nbsp;" . give_get_price_option_name( $item['id'], $price_id, $payment_id );
				}
			...
			if ( ! empty( $files ) ) {
				...
				$bundled_products = apply_filters( 'give_email_tag_bundled_products', give_get_bundled_products( $item['id'] ), $item, $payment_id, 'download_list' );

				foreach ( $bundled_products as $bundle_item ) {

					$download_list .= '<div class="give_bundled_product"><strong>' . get_the_title( $bundle_item ) . '</strong></div>';
 ...
 * @return string download_list
 */
function give_email_tag_download_list_plain( $payment_id ) {
	...
	if ( $cart_items ) {
		...
		foreach ( $cart_items as $item ) {
			...
			if ( $show_names ) {
				...
				if ( ! empty( $quantity ) && $quantity > 1 ) {
					$title .= __( 'Quantity', 'give' ) . ': ' . $quantity;
				}

				if ( ! empty( $sku ) ) {
					$title .= __( 'SKU', 'give' ) . ': ' . $sku;
				}
				...
				$download_list .= apply_filters( 'give_email_receipt_download_title', $title, $item, $price_id, $payment_id )  . "\n";
			}
			...
			if ( ! empty( $files ) ) {
				...
				foreach ( $files as $filekey => $file ) {
					...
					$download_list .= "\n";
					$file_url = give_get_download_file_url( $payment_data['key'], $email, $filekey, $item['id'], $price_id );
					$download_list .= give_get_file_name( $file ) . ': ' . $file_url . "\n";
			...
			if ( '' != give_get_product_notes( $item['id'] ) ) {
				$download_list .= "\n";
				$download_list .= give_get_product_notes( $item['id'] ) . "\n";
			}
		}
	}
...
function give_email_tag_file_urls( $payment_id ) {
	...
	foreach ( $cart_items as $item ) {
		...
		elseif ( give_is_bundled_product( $item['id'] ) ) {
			...
			$bundled_products = apply_filters( 'give_email_tag_bundled_products', give_get_bundled_products( $item['id'] ), $item, $payment_id, 'file_urls' );
...
function give_email_tag_tax( $payment_id ) {
	$tax = give_currency_filter( give_format_amount( give_get_payment_tax( $payment_id ) ), give_get_payment_currency_code( $payment_id ) );
