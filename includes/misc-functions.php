...
if ( ! function_exists( 'array_column' ) ) {
	...
	function array_column( $input = null, $columnKey = null, $indexKey = null ) {
		...
		if ( $argc < 2 ) {
			trigger_error( sprintf( __( 'array_column() expects at least 2 parameters, %s given.', 'give' ), $argc ), E_USER_WARNING );

			return null;
		}

		if ( ! is_array( $params[0] ) ) {
			trigger_error( sprintf( __( 'array_column() expects parameter 1 to be array, %s given.', 'give' ), gettype( $params[0] ) ), E_USER_WARNING );
		...
		if ( ! is_int( $params[1] )
			...
			trigger_error( __( 'array_column(): The column key should be either a string or an integer.', 'give' ), E_USER_WARNING );
		...
		if ( isset( $params[2] )
			...
			trigger_error( __( 'array_column(): The index key should be either a string or an integer.', 'give' ), E_USER_WARNING );
