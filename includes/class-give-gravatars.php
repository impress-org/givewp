...
class Give_Donators_Gravatars {
	...
	function gravatars( $form_id = false, $title = '' ) {
		...
		if ( isset ( $title ) ) {
			...
			if ( $title ) {
				echo apply_filters( 'give_donators_gravatars_title', '<h3 class="give-gravatars-title">' . esc_html__( $title ) . '</h3>' );
			} elseif ( isset( $give_options['give_donators_gravatars_heading'] ) ) {
				echo apply_filters( 'give_donators_gravatars_title', '<h3 class="give-gravatars-title">' . esc_html__( $give_options['give_donators_gravatars_heading'] ) . '</h2>' );
			}
