...
function give_process_donation_form()
{
    ...
    $price = isset($post_data['give-amount']) ?
        (float)apply_filters('give_donation_total',
            give_maybe_sanitize_amount($post_data['give-amount'], ['currency' => give_get_currency($form_id)])) :
        '0.00';
        ...
        if ($found_donor->id > 0) {
            give_set_error(
                ...
                /* translators: 1. Donor Email, 2. Submitted Email */
                    __('You are logged in as %1$s, and are submitting a donation as %2$s, which is an existing donor. To ensure that the email address is tied to the correct donor, please submit this donation from a logged-out browser, or choose another email address.',
                        'give'),
                    $donor->email,
                    $submitted_email
                )
...
function give_donation_form_validate_fields()
{
    ...
    if (
        ...
        give_set_error('spam_donation',
            __('The email you are using has been flagged as one used in SPAM comments or donations by our system. Please try using a different email address or contact the site administrator if you have any questions.',
                'give'));
    }
...
function give_verify_minimum_price($amount_range = 'minimum')
{
    $post_data = give_clean($_POST); // WPCS: input var ok, sanitization ok, CSRF ok.
    $form_id = ! empty($post_data['give-form-id']) ? $post_data['give-form-id'] : 0;
    $amount = ! empty($post_data['give-amount']) ? give_maybe_sanitize_amount($post_data['give-amount'],
        ['currency' => give_get_currency($form_id)]) : 0;
...
function give_donation_form_validate_agree_to_terms()
{
    ...
    if ( ! $agree_to_terms) {
        // User did not agree.
        give_set_error('agree_to_terms',
            apply_filters('give_agree_to_terms_text', __('You must agree to the terms and conditions.', 'give')));
    }
}
...
function give_donation_form_validate_new_user()
{
    ...
    if ( ! wp_verify_nonce($nonce, "give_form_create_user_nonce_{$form_id}")) {
        give_set_error('invalid_nonce',
            __('We\'re unable to recognize your session. Please refresh the screen to try again; otherwise contact your website administrator for assistance.',
                'give'));
    }
...
function give_register_and_login_new_user($user_data = [])
{
    ...
    $user_args = apply_filters(
        ...
        [
            'user_login' => isset($user_data['user_login'])
                ? sanitize_user($user_data['user_login'])
                : '',
            'user_pass' => isset($user_data['user_pass']) ? $user_data['user_pass'] : '',
            'user_email' => isset($user_data['user_email'])
                ? sanitize_email($user_data['user_email'])
                : '',
            'first_name' => isset($user_data['user_first'])
                ? sanitize_text_field($user_data['user_first'])
                : '',
            'last_name' => isset($user_data['user_last'])
                ? sanitize_text_field($user_data['user_last'])
                : '',
...
function give_donation_form_validate_cc()
{
    ...
    if ( ! empty($card_data['card_zip'])) {
        if ( ! give_donation_form_validate_cc_zip($card_data['card_zip'], $card_data['card_country'])) {
            give_set_error('invalid_cc_zip',
                __('The zip / postal code you entered for your billing address is invalid.', 'give'));
        }
    }
...
function give_validate_donation_amount($valid_data)
{
    ...
    if ($form->is_set_type_donation_form()) {
        ...
        $post_data['give-amount'] = give_maybe_sanitize_amount($post_data['give-amount'],
            ['currency' => $form_currency]);
        $variable_price_option_amount = give_maybe_sanitize_amount(give_get_price_option_amount($post_data['give-form-id'],
            $post_data['give-price-id']), ['currency' => $form_currency]);
        $new_price_id = '';
...
function give_donation_form_validate_name_fields($post_data)
{
    ...
    if ( ! $is_alpha_first_name || ( ! empty($post_data['give_last']) && ! $is_alpha_last_name)) {
        give_set_error('invalid_name',
            esc_html__('The First Name and Last Name fields cannot contain an email address or numbers.', 'give'));
    }
}
