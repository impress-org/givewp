...
class Give_Donation_History {
	...
	/**
	 * Send mail link to donor mail id
	 *
	 * @since 1.8.17
	 */
	public function donation_history_send_confirmation() {
		...
		if ( check_ajax_referer( 'give_donation_history', 'nonce', false ) ) {
			$donor = Give()->donors->get_donor_by( 'email', $email );

			if ( ! empty( $donor->id ) && ! empty( $email ) ) {
				$this->send_confirmation_mail( $donor->id, $email );
	...
	/**
	 * Donation history template email.
	 *
	 * @since 1.8.17
	 *
	 ...
	 * @param string $email
	 */
	public function send_confirmation_mail( $customer_id = false, $email = '' ) {
