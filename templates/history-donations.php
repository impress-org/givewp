		...
		<?php foreach ( $donations as $post ) : setup_postdata( $post ); ?>
			...
			<tr class="give_purchase_row">
				...
				<td class="give_purchase_details">
					...
					<a href="<?php echo add_query_arg( 'payment_key', give_get_payment_key( $post->ID ), give_get_success_page_uri() ); ?>">&raquo;</a>
					<?php else: ?>
					<a href="<?php echo add_query_arg( 'payment_key', give_get_payment_key( $post->ID ), give_get_success_page_uri() ); ?>"><?php _e( 'View Details', 'give' ); ?></a>
					<?php endif; ?>
				</td>
...
<?php else : ?>
	<p class="give-no-purchases"><?php _e('You have not made any purchases', 'give'); ?></p>
<?php endif;
