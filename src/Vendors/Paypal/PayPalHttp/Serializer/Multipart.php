...
class Multipart implements \Give\Vendor\Paypal\PayPalHttp\Serializer {
	...
	private function prepareFormField( $partName, $value, $boundary ) {
		return \implode( self::LINEFEED, [ "Content-Disposition: form-data; name=\"{$partName}\"", '', \filter_var( $value ) ] );
	}
	private function prepareFilePart( $partName, $file, $boundary ) {
		...
		return \implode( self::LINEFEED, [ "Content-Disposition: form-data; name=\"{$partName}\"; filename=\"{$filePath}\"", "Content-Type: {$mimeType}", '', $data ] );
	}
